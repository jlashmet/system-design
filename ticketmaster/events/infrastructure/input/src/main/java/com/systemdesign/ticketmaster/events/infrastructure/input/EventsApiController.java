...
import com.systemdesign.ticketmaster.events.application.CreateEventCommand;
...
public final class EventsApiController implements EventsApi {
    ...
    public EventsApiController(CreateEventHandler createEventHandler, GetEventHandler getEventHandler) {
    ...
    public ResponseEntity<EventResponse> createEvent(CreateEventRequest request) {
        Event event = createEventHandler.handle(new CreateEventCommand(
                new EventId(request.getEventId()),
                request.getName(),
                new VenueId(request.getVenueId()),
                request.getStartsAt().toInstant(),
                request.getCategory(),
                request.getDescription()));
        return ResponseEntity.created(URI.create("/events/" + event.id().value()))
