...
public final class StartCheckoutHandler {
    ...
    public StartCheckoutResult handle(StartCheckoutCommand command) {
        ...
        return findIdempotentBooking(command)
                .map(booking -> ensureSameRequest(command, booking))
    ...
    private StartCheckoutResult startNewCheckout(StartCheckoutCommand command) {
        ...
        PreparedCheckout prepared = reservationCheckoutService.prepareCheckout(
                command.eventId(),
                command.userId(),
                new LinkedHashSet<>(command.seatIds()),
                command.admissionToken(),
                now,
                now.plus(checkoutDuration));
        ReservationCheckout reservation = prepared.reservation();
        ...
        try {
            checkoutGateway.startCheckout(prepared, booking);
            return createAndPersistPaymentIntent(booking, reservation.checkoutExpiresAt());
        } catch (CheckoutConflictException conflict) {
            return findIdempotentBooking(command)
                    .map(bookingAfterConflict -> ensureSameRequest(command, bookingAfterConflict))
                    .map(this::ensurePaymentIntent)
    ...
    private Booking ensureSameRequest(StartCheckoutCommand command, Booking booking) {
        if (!booking.eventId().equals(command.eventId())
                || !booking.userId().equals(command.userId())
                || !booking.checkoutIdempotencyKey().equals(command.idempotencyKey())) {
            throw new IllegalStateException("checkout idempotency mapping resolved outside its event/user/key scope");
        }
        ...
        if (!reservation.seatIds().equals(new LinkedHashSet<>(command.seatIds()))) {
            throw new CheckoutIdempotencyConflictException(command.idempotencyKey());
        }
        return booking;
    }
    ...
    private static void requirePaymentReservationScope(
        ...
        if (!reservation.userId().equals(booking.userId())) {
            throw new IllegalStateException("checkout reservation owner mismatch for " + booking.id().value());
        }
